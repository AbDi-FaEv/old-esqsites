 ...
 * @version SVN: $Id: sfPropelObjectPeerImpersonator.class.php 21755 2009-09-07 18:53:32Z Dannyrulez $
 */
...
class sfPropelObjectPeerImpersonator
{
  ...
  protected
    ...
    $currentStartColumnForPropelObjects = 0,
   ...
   * Object population
   */
  public function populateObjects($stmt)
  {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_NUM))
    {
      ...
      foreach ($this->objects as $index => $object)
      {
        ...
        if (self::isPropelObject($object))
        {
          $rowObjects[$index]->hydrate($row, $this->objectsStartColumns[$index]);
        }
        else
        {
          $rowObjects[$index]->hydrate($row, $this->currentStartColumnForPropelObjects + $currentImpersonatedObjectsStartColumn);
  ...
  {
    if ($this->securityFieldCountFlag && ($_acceptable=($this->currentStartColumnForPropelObjects + $this->currentStartColumnForImpersonatedObjects)) != ($_got = count($c->getSelectColumns())+count($c->getAsColumns())))
    {
    ...
    return $this->populateObjects($this->doSelectStmt($c, $con));
  }

  /**
   * Impersonates doSelectStmt, to call behaviours callables
   */
  public function doSelectStmt(Criteria $criteria, $con = null)
  {
    $basePeer = 'Base'.get_class($this->objects[0]).'Peer';

    foreach (sfMixer::getCallables($basePeer.':addDoSelectStmt:addDoSelectStmt') as $callable)
    {
  ...
  public function doCount(Criteria $criteria, $distinct = false, $con=null, $usePeerClassSelectResultSetMethod=true)
  {
    ...
    {
      // kept for compatibility, but maybe useless. Problem with this is that doSelectStmt can modify
      ...
      $stmt = call_user_func(array($peerClass, 'doSelectStmt'), $criteria, $con);
    }
    else
    {
