...
jQuery.fn.multiSelect = function(to, options) {
	...
	var moveUpFunction = function() { 
		jQuery( "option:selected", $select ).each( 
			function() {
				var prev = jQuery(this).prev();
				var tmp = jQuery(this).remove();
				prev.before(tmp);
			});
		};
	
	var moveDownFunction = function() { 
		jQuery( "option:selected", $select ).each( 
			function() {
	...
	if(options.moveUp)
		jQuery(options.moveUp).click(moveUpFunction);
	
	if(options.moveDown)
		jQuery(options.moveDown).click(moveDownFunction);
	...
	function moveOptions(from, toSel, beforeMove, afterMove) {
      if (beforeMove && !beforeMove())
			return;
		jQuery("option:selected", toSel).attr('selected', false);
