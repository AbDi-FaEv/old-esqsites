...
abstract class BasesfGuardPermission extends BaseObject  implements Persistent
{
	...
	public function setTitle($v)
	{
		...
		if ($this->title !== $v) {
			$this->title = $v;
			$this->modifiedColumns[] = sfGuardPermissionPeer::TITLE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = sfGuardPermissionPeer::NUM_COLUMNS - sfGuardPermissionPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(sfGuardPermissionPeer::ID) ) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(sfGuardPermissionPeer::TITLE)) $criteria->add(sfGuardPermissionPeer::TITLE, $this->title);
