...
abstract class BaseTagging extends BaseObject  implements Persistent
{
	 ...
	 * @return     Tagging The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->tag_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->taggable_model = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->taggable_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->resetModified();
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = TaggingPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				TaggingPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(TaggingPeer::ID) ) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		if ($includeForeignObjects) {
			if (null !== $this->aTag) {
				$result['Tag'] = $this->aTag->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function clear()
	{
		$this->id = null;
		$this->tag_id = null;
		$this->taggable_model = null;
		$this->taggable_id = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseTagging:' . $name))
		{
