...
class checkoutActions extends sfActions
{
  ...
  {
    if(! $this->getUser()->getTemporaryWebsite()->getWebsiteTemplate() instanceof WebsiteTemplate){
        $defaultTemplate = WebsiteTemplateQuery::create()->filterById("74532781f16a9b074cfca0b8b225a0f1")->findOne();
        $this->getUser()->getTemporaryWebsite()->setWebsiteTemplate($defaultTemplate);
    }
  ...
  {
    //$step = $this -> steps[CheckoutSteps::STEP_GALLERY]; //Removed to bypass redirect
    if(! $this->getUser()->getTemporaryWebsite()->getWebsiteTemplate() instanceof WebsiteTemplate){
        $defaultTemplate = WebsiteTemplateQuery::create()->filterById("74532781f16a9b074cfca0b8b225a0f1")->findOne();
        $this->getUser()->getTemporaryWebsite()->setWebsiteTemplate($defaultTemplate);
    }
    $step = $this->steps[CheckoutSteps::STEP_INFO];
     if(!$step -> isCompleted()) //Need the info
     {
       $this -> forceStep($step);
     }
  ...
  {
    $wTemplate = $this->getUser()->getTemporaryWebsite()->getWebsiteTemplate();

    if (! $wTemplate instanceof WebsiteTemplate) {
        $defaultTemplate = WebsiteTemplateQuery::create()->filterById("74532781f16a9b074cfca0b8b225a0f1")->findOne();
        //$templateQuery->filterById("d6fdface82c4d2d72e89e49ac85227ad");
        $this->getUser()->getTemporaryWebsite()->setWebsiteTemplate($defaultTemplate);
    }

    if(($step = $this -> steps -> getHighest(false)) != $this -> steps[CheckoutSteps::STEP_CHECKOUT] && ($step != $this->steps[CheckoutSteps::STEP_GALLERY]))
    {
      $this -> forceStep($step);
    }
